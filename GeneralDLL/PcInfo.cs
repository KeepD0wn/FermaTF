...
namespace GeneralDLL
{
    public class PcInfo
    {
        public static string salt = "3Z=>/&qz,<'.@";

        /// <summary>
        /// возвращает хэшированную строку с данными с текущего компьютера
        /// </summary>
        /// <returns></returns>
        public static string GetCurrentPCInfo()
        {
            ...
            try
            {
                ...
                byte[] checkSum = md5.ComputeHash(Encoding.UTF8.GetBytes(final+salt));
            ...
            {
                Logger.LogAndWritelineAsync("[017] Error");
                Thread.Sleep(5000);
                Environment.Exit(0);
            }
            return result;
